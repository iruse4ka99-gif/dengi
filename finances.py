...
st.markdown(f"""
    ...
    .m-val {{ font-size: 20px; font-weight: 800; color: #1d1d1f; }}
    ...
    .p-bar-bg {{ width: 100%; height: 8px; background: rgba(0,0,0,0.06); border-radius: 10px; margin: 15px 0; }}
...
df = pd.DataFrame() # Заглушка, чтобы не было NameError
try:
...
if st.session_state.page == "wallet":
    ...
    with c_f:
        with st.form("add", clear_on_submit=True):
            ...
            amt = st.number_input("Сумма ₪", min_value=0, step=10)
    ...
    with st.form("transfer"):
        f = st.selectbox("Откуда?", list(envelopes.keys())); t = st.selectbox("Куда?", list(envelopes.keys()))
        v = st.number_input("₪", min_value=1)
        if st.form_submit_button("ВЫПОЛНИТЬ"):
            ...
            conn.update(worksheet="Data", data=u_df); st.success("ОК!"); st.rerun()
    ...
    if st.button("🏁 ЗАКРЫТЬ МЕСЯЦ"):
        ...
        conn.update(worksheet="Data", data=f_df); st.balloons(); st.rerun()
    st.divider()
